...
namespace DocsContext.Storage.Sqlite;
...
) : IPackageStore
{
    ...
    )
    {
        ...
        DocsContextTelemetry.Metrics.PackagesInstalled.Add(1);
        DocsContextTelemetry.Metrics.BuildDurationMs.Record(
    ...
    )
    {
        ...
        if (manifest is null)
        {
            throw new InvalidOperationException(
                "Package file does not contain DocsContext manifest metadata."
            );
        }
    ...
    {
        using var activity = DocsContextTelemetry.ActivitySource.StartActivity(
            DocsContextTelemetry.Activities.Search
        );
        activity?.SetTag("docscontext.package_id", request.PackageId);
...
LIMIT $limit;";
        ...
        DocsContextTelemetry.Metrics.ResultsReturned.Add(selectedHits.Count);
        DocsContextTelemetry.Metrics.SearchDurationMs.Record(
            (DateTimeOffset.UtcNow - startedAt).TotalMilliseconds
        );
...
FROM chunks;",
    ...
    )
    {
        ...
        foreach (var hit in hits)
        {
            ...
            {
                DocsContextTelemetry.Metrics.TokenBudgetTrims.Add(1);
                break;
            }
