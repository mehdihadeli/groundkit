...
public sealed class PackageLifecycleTests : IDisposable
{
    ...
    [Fact]
    public async Task BuildAndSave_LocalDirectory_CanQueryContent()
    {
        ...
        Assert.Contains(
            response.Hits,
            hit => hit.Content.Contains("local-first", StringComparison.OrdinalIgnoreCase)
        );
    }

    [Fact]
    public async Task ExportAndImport_RoundTripsPackage()
    {
        var docsPath = Path.Combine(_tempRoot, "docs");
        Directory.CreateDirectory(docsPath);
    ...
    [Fact]
    public async Task Remove_DeletesPackageFile()
    {
        var docsPath = Path.Combine(_tempRoot, "docs");
        Directory.CreateDirectory(docsPath);
        await File.WriteAllTextAsync(Path.Combine(docsPath, "page.md"), "# Page\n\nContent.");
    ...
    [Fact]
    public async Task QueryAsync_TokenBudget_TrimsHits()
    {
        var docsPath = Path.Combine(_tempRoot, "docs");
        Directory.CreateDirectory(docsPath);
        await File.WriteAllTextAsync(
        ...
        Assert.True(response.Hits.Count >= 1);
        Assert.True(response.TotalTokens <= 150);
    }
